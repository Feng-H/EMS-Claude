...
- **标准化工具集 (Tool Discovery)**: 提供 MCP 兼容的工具 Schema，支持多级 Scope 授权（读设备、写报修、读库存等），确保 Agent 按需访问。
- **工业级安全网关**: API Key 采用 SHA-256 哈希存储，支持自定义 **Scopes 权限范围** 与 **Rate Limiting 频控**（分钟级），保障生产系统安全。
- **工厂级多租户隔离**: 严密的权限隔离逻辑，Agent 审计与工具调用自动限定在用户所属工厂/车间范围内，防止跨工厂数据泄露。
- **主动事件推送 (Proactive Push)**: 支持 Webhook 订阅，内置签名机制。当发生 RUL 风险预警、点检 NG 或备件低库存时，系统实时推送结构化 Payload。
- **混合 RAG 与动态分析**: 结合专家知识库与技术手册分片，支持 **PDF 手册上传、自动文本提取与智能分片**。
- **Text-to-SQL 受控分析**: 引入 `sql_data_analyst` 工具，支持 Agent 自主生成并执行只读 SQL，实现跨表交叉维度的数据探索与分析，同时内置安全拦截与工厂隔离机制。
